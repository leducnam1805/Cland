					      ...
					      <c:forEach begin="1" end="${totalPage}" var="i">
					      	<c:choose>
					      		<c:when test="${not empty search}">
					      			<li class="<c:if test="${i == currentPage}">active</c:if>"><a href="${pageContext.request.contextPath}/admin/land/index/${i}?q=${search}">${i}<span class="sr-only">(current)</span></a></li>
					      		</c:when>
					      		<c:otherwise>
